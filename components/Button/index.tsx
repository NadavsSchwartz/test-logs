...
import { applyStyleToMultipleVariants } from './cvs';
...
const buttonClasses = cva(
  'flex justify-center items-center text-sm font-medium relative rounded-md transition-colors',
  {
    variants: {
      color: {
        primary: 'text-white dark:text-black bg-brand-500 ',
        secondary:
          'text-gray-900 dark:text-darkgray-900 bg-gray-200 dark:bg-darkgray-100',
        minimal: 'text-gray-900 dark:text-black',
        destructive: '',
      },
      size: {
    ...
    compoundVariants: [
      ...
      },
      {
        loading: true,
        color: 'primary',
        className: 'bg-gray-800/30 text-white/30 ',
      },
      ...applyStyleToMultipleVariants({
        disabled: [undefined, false],
        color: 'primary',
        className:
          'bg-brand-500 hover:bg-brand-400 focus:border focus:border-white focus:outline-none   focus:ring-offset focus:ring-brand-500 ',
      }),
      // Secondary variants
      {
        disabled: true,
      ...
      ...applyStyleToMultipleVariants({
        ...
        color: 'secondary',
        className:
          'border border-gray-300 dark:border-darkgray-300 hover:bg-gray-50 hover:border-gray-400 focus:bg-gray-100 dark:hover:bg-darkgray-200 dark:focus:bg-darkgray-200 focus:outline-none   focus:ring-offset focus:ring-gray-900 dark:focus:ring-white',
      }),
      // Minimal variants
      {
        disabled: true,
      ...
      ...applyStyleToMultipleVariants({
        ...
        color: 'minimal',
        className:
          'hover:bg-gray-100 focus:bg-gray-100 dark:hover:bg-white  focus:outline-none   focus:ring-offset focus:ring-gray-900  ',
      }),
      ...
      ...applyStyleToMultipleVariants({
        ...
        color: 'destructive',
        className:
          'text-gray-900 hover:text-red-700 focus:text-red-700 dark:hover:text-red-700 dark:focus:text-red-700 hover:border-red-100 focus:border-red-100 hover:bg-red-100 focus:bg-red-100 focus:outline-none   focus:ring-offset focus:ring-red-700',
      }),
    ],
    defaultVariants: {
      color: 'primary',
...
>(function Button(props: ButtonProps, forwardedRef) {
  ...
  // If an `href`-attr is passed it's `<a>`, otherwise it's a `<button />`
    ...
    <>
      {StartIcon && (
        <StartIcon
          className={classNames(
            'inline-flex',
            size === 'icon' ? 'h-4 w-4' : 'h-4 w-4 stroke-[1.5px]'
          )}
        />
      )}
      {props.children}
      {loading && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform">
          <svg
            className="mx-4 h-5 w-5 animate-spin text-black"
      ...
      {EndIcon && <EndIcon className="inline h-5 w-5" />}
    </>
  );
