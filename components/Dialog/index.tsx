...
import { SVGComponent } from '../Layout/Shell';
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          ...
          className={classNames(
            'fadeIn fixed left-1/2 top-1/2 z-50 min-w-[270px] -translate-x-1/2 -translate-y-1/2 rounded bg-white text-left shadow-xl focus-visible:outline-none sm:align-middle',
            props.size == 'xl'
              ? 'p-8 sm:max-w-[90rem]'
              : props.size == 'lg'
              ? 'p-8 sm:max-w-[70rem]'
              ...
              ? 'p-8 sm:max-w-[48rem]'
              : 'p-8 sm:max-w-[35rem]',
            props.size == 'sm' ? 'p-4 sm:max-w-[25rem]' : 'max-h-[95vh]',
...
export function DialogHeader(props: DialogHeaderProps) {
  ...
  return (
    <div className="mb-4">
      <h3
        className="leading-20 text-semibold  pb-1 text-xl text-gray-900"
        id="modal-title"
      >
        {props.title}
      </h3>
...
export function DialogFooter(props: { children: ReactNode }) {
  return (
    <div className="mt-7 flex justify-end space-x-2 ">{props.children}</div>
  );
}
